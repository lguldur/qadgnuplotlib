/*
    ...
    Purpose:
        Local plot glue/adaptation layer for running gnuplot scripts inside the embedded library.
...
/*
 * qadgnuplotlib wrapper hook.
 *
 * command_line_env is intentionally static in this translation unit, so the
 * embedded library must install the error trap here rather than trying to
 * access the jump buffer from qadgnuplotlib.c.
 */
int
qadgnuplotlib_do_line_protected(int* did_longjmp)
{
    JMP_BUF keepenv;
    int rc;
