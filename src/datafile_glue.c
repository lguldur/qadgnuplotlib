/*
    ...
    Purpose:
        Local datafile glue/adaptation layer adding mem:// registered-memory input support while preserving the gnuplot datafile pipeline.
...
/*
 * qadgnuplotlib local hook.
 *
 * This file is derived from upstream gnuplot datafile.c.  Upstream files remain
 * untouched; this local copy adds the mem:// data source.
 *
 * mem://x,y,z is backed by registered qadgnuplotlib::Context columns.  The data
 * is read directly from borrowed C++ memory through this small C ABI; no
 * temporary file and no generated ASCII stream are used.
 */
extern void *qadgnuplotlib_mem_open(const char *uri, char **errmsg);
extern void qadgnuplotlib_mem_close(void *handle);
extern size_t qadgnuplotlib_mem_row_count(const void *handle);
extern size_t qadgnuplotlib_mem_column_count(const void *handle);
extern int qadgnuplotlib_mem_read_row(const void *handle, size_t row, double *out, size_t out_count);
extern void qadgnuplotlib_mem_free(void *ptr);
...
static int df_readmem(double v[], int max);
...
static void *mem_data_handle = NULL;
static size_t mem_data_row = 0;
static size_t mem_data_row_count = 0;
static size_t mem_data_column_count = 0;
#if defined(PIPES)
...
df_open(const char *cmd_filename, int max_using, struct curve_points *plot)
{
    ...
    if (!strncmp(df_filename, "mem://", 6)) {
	char *errmsg = NULL;

	if (df_binary_file || df_matrix_file)
	    int_error(name_token, "mem:// currently supports simple registered column data only");

	mem_data_handle = qadgnuplotlib_mem_open(df_filename, &errmsg);
	if (!mem_data_handle) {
	    if (errmsg) {
		char *tmp = gp_strdup(errmsg);
		qadgnuplotlib_mem_free(errmsg);
		int_error(name_token, "%s", tmp);
		free(tmp);
	    } else {
		int_error(name_token, "mem:// data source unavailable");
	    }
	}
	mem_data_row = 0;
	mem_data_row_count = qadgnuplotlib_mem_row_count(mem_data_handle);
	mem_data_column_count = qadgnuplotlib_mem_column_count(mem_data_handle);
	volatile_data = TRUE;
...
df_close()
{
    ...
    if (!data_fp && !df_datablock && !mem_data_handle)
	return;
    ...
    if (mem_data_handle) {
	qadgnuplotlib_mem_close(mem_data_handle);
	mem_data_handle = NULL;
	mem_data_row = 0;
	mem_data_row_count = 0;
	mem_data_column_count = 0;
    }
...
/*{{{  int df_readmem(double v[], int max) */
/*
 * Direct reader for qadgnuplotlib mem:// registered columns.
 *
 * This intentionally starts with the common/simple cases:
 *   plot  "mem://y"
 *   plot  "mem://x,y"
 *   splot "mem://x,y,z"
 * and optional numeric "using" columns such as "using 1:2".
 *
 * The path avoids temporary files and ASCII generation.  Values are converted
 * to double on demand by the C++ registry adapter.  More advanced datafile
 * features (string columns, tic-label strings, time parsing from strings,
 * matrix/image mode) can be added later without changing the public URI syntax.
 */
static int
df_readmem(double v[], int max)
{
    int output;
    ...
    if (!mem_data_handle || mem_data_row >= mem_data_row_count) {
	df_no_cols = 0;
	df_eof = 1;
	return DF_EOF;
    }

    while (mem_data_row < mem_data_row_count) {
	TBOOLEAN line_okay = TRUE;
	size_t col_count = mem_data_column_count;

	if (col_count > (size_t)MAXDATACOLS)
	    col_count = MAXDATACOLS;
	if (df_max_cols < (int)col_count)
	    expand_df_column((int)col_count);

	df_no_cols = (int)col_count;

	if (!qadgnuplotlib_mem_read_row(mem_data_handle,
					mem_data_row,
					v,
					col_count)) {
	    df_eof = 1;
	...
	/* Single-block data source: no blank lines or indexes. */
	df_last_index_read = df_current_index;
	blank_count = 0;

	/* Honour the common 'every' point filtering.  The line fields are
	 * normally zero for a single continuous block, matching ordinary files.
	 */
	...
	/* Mirror enough of df_column[] for $N / column(N) in simple using
	 * expressions.  There is no backing string position for mem:// values.
	 */
	{
	    int i;
	    for (i = 0; i < df_no_cols; ++i) {
		df_column[i].datum = v[i];
	...
	/* copy column[] to v[] via use[] */
	{
		...
		||  use_spec[output].expected_type == CT_KEYLABEL
		||  use_spec[output].expected_type == CT_STRING) {
		    int_error(NO_CARET,
			"mem:// currently supports numeric columns only");
		}
...
{
    if (!data_fp && !df_pseudodata && !df_datablock && !df_array && !mem_data_handle)
	return DF_EOF;

    if (mem_data_handle) {
	return df_readmem(v, max);
