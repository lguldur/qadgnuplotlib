/*
    ...
    Purpose:
        Local glue/adaptation layer for embedded gnuplot integration.
        Appends the qadgnuplotlib library version to gnuplot's native
        "show version" and "show version long" output.
...
static void
show_qadgnuplotlib_version(FILE *fp, const char *prefix, int verbose)
{
    const char *version = qadgnuplotlib_version(verbose);
...
show_version(FILE *fp)
{
    ...
    char *p = prefix;
    char fmt[2048];
    int show_long = almost_equals(c_token, "l$ong");
...
# endif
    ...
    show_qadgnuplotlib_version(fp, p, show_long);
}
